...
import { copySync } from '@polkadot/dev/scripts/copy.mjs';
import { denoCreateName, denoExtPrefix, denoIntPrefix } from '@polkadot/dev/scripts/deno.mjs';
    ...
    try {
      return [
        JSON
          ...
          .references
          .map(({ path }) =>
            path.replace(/\.\.\/\//g, '').replace('tsconfig.build.json', '')
          ),
        true
      ];
    } catch (error) {
